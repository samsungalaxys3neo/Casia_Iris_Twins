...
OUTPUT_DIR = PROJECT_ROOT / "data/normalized/strict"
...
def rubber_sheet_normalize(gray, row, radial_res=64, angular_res=512):
    ...
    # Bordo esterno iride/limbus
    xi = iris_x + iris_r * np.cos(theta)
    yi = iris_y + iris_r * np.sin(theta)

    # Coordinate radiali normalizzate da 0 a 1
    ...
    # Maschera base: coordinate dentro l'immagine
    valid = (
        (map_x >= 0)
        & (map_x < w)
        & (map_y >= 0)
    ...
    # Maschera semplice per riflessi/saturazione.
    # Non eliminiamo troppo: evitiamo solo valori estremi.
...
def make_contact_sheet(df, image_col, title, output_path, n=45):
    ...
    for _, row in df.iterrows():
        ...
        label2 = f"valid={row['valid_ratio']:.3f}"
...
def main():
    ...
    if len(ok_df) > 0:
        summary["valid_ratio"] = {
            "min": float(ok_df["valid_ratio"].min()),
            "q1": float(ok_df["valid_ratio"].quantile(0.25)),
            "median": float(ok_df["valid_ratio"].median()),
            "mean": float(ok_df["valid_ratio"].mean()),
            "q3": float(ok_df["valid_ratio"].quantile(0.75)),
            "max": float(ok_df["valid_ratio"].max()),
        }

    with SUMMARY_PATH.open("w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    # Contact sheet immagini normalizzate
    make_contact_sheet(
        ok_df,
    ...
    # Contact sheet casi con valid_ratio più basso
