...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "normalized" / "daugman_strict_v3"
...
def project_relative_or_absolute(path: Path) -> str:
    """
    Used for paths saved inside CSV files.
    If the path is inside the project, store it as a project-relative path.
...
def main():
    ...
    parser.add_argument(
        ...
        help="Cartella output. Default: data/normalized/daugman_strict_v3",
    )
